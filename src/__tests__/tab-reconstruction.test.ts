 ...
 * Excludes sender (localPublicKey) from recipients for normalization
 */
function extractGroupTabs(messages: Message[], localPublicKey?: string): Map<string, string[]> {
    const groupRecipients = new Map<string, string[]>();
    for (const msg of messages) {
        if (msg.to && msg.to.length > 1) {
            // Normalize: exclude sender if provided, deduplicate, and sort
            const filtered = localPublicKey ? msg.to.filter(id => id !== localPublicKey) : msg.to;
            const sorted = Array.from(new Set(filtered)).sort();
            // Only create group tab if 2+ recipients remain
            if (sorted.length > 1) {
...
describe('Tab reconstruction on reload', () => {
    ...
    describe('Group tab extraction', () => {
        ...
        it('should exclude sender from group tabs when localPublicKey provided', () => {
            const messages: Message[] = [
                // Local user sends to bob and carol
                { from: 'me@44444444', text: 'Group msg', timestamp: 1000, to: [bobKey, carolKey, localKey] },
                // Bob sends to local and carol - same group after normalization
                { from: 'bob@22222222', text: 'Reply', timestamp: 1001, to: [localKey, carolKey] },
            ];

            const groupTabs = extractGroupTabs(messages, localKey);

            // Should be 1 group: bob+carol (localKey excluded from both)
            expect(groupTabs.size).toBe(1);
            const key = [bobKey, carolKey].sort().join('|');
            expect(groupTabs.has(key)).toBe(true);
        });

        it('should handle messages where sender is only other recipient', () => {
            const messages: Message[] = [
                // Message to self and one other - becomes DM after sender excluded
                { from: 'me@44444444', text: 'To bob and me', timestamp: 1000, to: [bobKey, localKey] },
            ];

            const groupTabs = extractGroupTabs(messages, localKey);

            // After excluding localKey, only bobKey remains (length 1), so not a group
            expect(groupTabs.size).toBe(0);
        });
    });
