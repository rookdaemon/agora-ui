...
class LocalInboundMessageGuard {
	...
	private isRateLimited(senderPublicKey: string): boolean {
		...
		if (!current && this.senderWindows.size >= LocalInboundMessageGuard.MAX_SENDER_ENTRIES) {
	...
	private isDuplicateContent(senderPublicKey: string, type: string, payload: unknown): boolean {
		...
		if (this.contentDedup.size > LocalInboundMessageGuard.MAX_CONTENT_ENTRIES) {
...
const coreGuard = (Agora as unknown as { InboundMessageGuard?: typeof LocalInboundMessageGuard }).InboundMessageGuard;
export const InboundMessageGuard = coreGuard ?? LocalInboundMessageGuard;
